    ...
    try {
        ...
        document.getElementById('btnSalvarDiario').addEventListener('click', async () => {
            ...
            try {
                ...
                // Se a turma ainda estava com inscrições abertas, muda para Em Andamento
                if (turmaData.status === 'Inscrições Abertas') {
                    await updateDoc(turmaRef, { status: 'Em Andamento' });
                    showAlertModal("Diário salvo! O status da turma agora é 'Em Andamento'.", false);
                } else {
